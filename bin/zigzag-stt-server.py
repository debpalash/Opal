#!/usr/bin/env python3
"""zigzag persistent STT server (Unix socket).
...
def main() -> int:
    ...
    # Fast dependency preflight (see zigzag-voice-server.py).
    ...
    srv.bind(SOCK_PATH)
    srv.listen(4)
    _eprint(f"zigzag-stt-server listening on {SOCK_PATH} (model={model_name})")
