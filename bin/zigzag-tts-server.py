...
Voice/speed come from env (ZIGZAG_TTS_VOICE / ZIGZAG_TTS_SPEED) so the
...
SOCK_PATH = "/tmp/zigzag-tts.sock"
OUT_WAV = "/tmp/zigzag_ai_tts.wav"
...
def _load_kitten():
    ...
    voice = os.environ.get("ZIGZAG_TTS_VOICE", "Bella")
    try:
        speed = float(os.environ.get("ZIGZAG_TTS_SPEED", "1.0"))
    except ValueError:
        speed = 1.0
    tts = KittenTTS()
...
def main() -> int:
    ...
    srv.bind(SOCK_PATH)
    srv.listen(4)
    _eprint(f"zigzag-tts-server listening on {SOCK_PATH}")
