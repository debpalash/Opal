...
fn httpGetRaw(url_str: []const u8, response_buf: []u8) !usize {
    ...
    var scratch = secure_temp.Workspace.create("language-http") catch return error.HttpFailed;
    defer scratch.cleanup();
    var tmp_path_buf: [secure_temp.max_path_len]u8 = undefined;
    const tmp_path = scratch.reserveFile("response", &tmp_path_buf) catch return error.HttpFailed;
...
fn ttsWorker(word_idx: usize) void {
    ...
    const secure_temp = @import("../core/secure_temp.zig");
    var scratch = secure_temp.Workspace.create("tts-word") catch return;
    defer scratch.cleanup();
    var wav_path_buf: [secure_temp.max_path_len]u8 = undefined;
    const wav_path = scratch.writeFile("speech.wav", wav_buf[0..body_len], &wav_path_buf) catch return;
...
fn ttsLineWorker() void {
    ...
    const secure_temp = @import("../core/secure_temp.zig");
    var scratch = secure_temp.Workspace.create("tts-line") catch return;
    defer scratch.cleanup();
    var wav_path_buf: [secure_temp.max_path_len]u8 = undefined;
    const wav_path = scratch.writeFile("speech.wav", wav_buf[0..body_len], &wav_path_buf) catch return;
...
fn asrWorker() void {
    ...
    const secure_temp = @import("../core/secure_temp.zig");
    var scratch = secure_temp.Workspace.create("language-asr") catch return;
    defer scratch.cleanup();
    var wav_path_buf: [secure_temp.max_path_len]u8 = undefined;
    const wav_path = scratch.reserveFile("audio.wav", &wav_path_buf) catch return;
    ...
    var extract_child = @import("../core/io_global.zig").Child.init(
        &.{ "ffmpeg", "-y", "-ss", start_str, "-i", media_path, "-t", "5", "-ar", "16000", "-ac", "1", "-f", "wav", wav_path },
        @import("../core/alloc.zig").allocator,
    );
    _ = extract_child.spawnAndWait() catch return;

    const wav_stat = @import("../core/io_global.zig").cwdStatFile(wav_path) catch return;
    if (wav_stat.size < 100) return;
    ...
    const resp_path = scratch.reserveFile("response.json", &resp_path_buf) catch return;
    // Build "@path" argument for curl --data-binary at runtime
    var data_arg_buf: [secure_temp.max_path_len + 1]u8 = undefined;
    const data_arg = std.fmt.bufPrint(&data_arg_buf, "@{s}", .{wav_path}) catch return;
    var curl_child = @import("../core/io_global.zig").Child.init(
        &.{ "curl", "-s", "-X", "POST", "-H", "Content-Type: audio/wav", "--data-binary", data_arg, "-o", resp_path, "--max-time", "30", url },
        @import("../core/alloc.zig").allocator,
    );
...
fn dubWorker() void {
    ...
    // Save WAV in a private per-operation workspace and remove it after audio
    // playback, including all early returns below.
    const secure_temp = @import("../core/secure_temp.zig");
    var scratch = secure_temp.Workspace.create("language-dub") catch return;
    defer scratch.cleanup();
    var dub_wav_path_buf: [secure_temp.max_path_len]u8 = undefined;
    const dub_wav_path = scratch.writeFile("dub.wav", wav_buf[0..wav_len], &dub_wav_path_buf) catch return;
...
pub fn saveSubtitleFlashcard() void {
    ...
    const line = std.fmt.bufPrint(&out_buf, "\"{s}\",\"{d:.1}\",\"{s}\",\"{s}\"\n", .{ source_name, time_pos, esc_src_buf[0..src_idx], esc_tl_buf[0..tl_idx] }) catch return;
