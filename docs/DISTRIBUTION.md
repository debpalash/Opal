...
The binary formula lives at [`Formula/opal.rb`](../Formula/opal.rb) and is
mirrored to the public `debpalash/homebrew-tap` repository.

- [x] Create the public `debpalash/homebrew-tap` repository.
- [x] Publish the prebuilt-binary formula with the release asset's verified
      SHA-256 (no Xcode/Zig source build).
- [x] Remove the retired `apfel` dependency; Opal's current AI backends are
      selected and installed by the app.
- [ ] Test on a clean machine: `brew install debpalash/tap/opal`.
- [x] Document in the README: `brew install debpalash/tap/opal`.
...
[`opal-media-player`](../packaging/aur/opal-media-player/PKGBUILD) (source) and
[`opal-media-player-bin`](../packaging/aur/opal-media-player-bin/PKGBUILD)
(prebuilt release). The release workflow derives `pkgver` from its tag,
refreshes checksums and `.SRCINFO`, and publishes each package independently.

- [x] Fetch tagged releases instead of a local working tree.
- [x] Require the project's Zig 0.16 toolchain for the source package.
- [x] Use unique AUR names; `opal` belongs to an unrelated telephony project.
- [x] Provide separate source and prebuilt-binary recipes.
- [ ] Add the workflow SSH public key to an AUR account, then claim the names:
      `git clone ssh://aur@aur.archlinux.org/opal-media-player.git`
      (cloning an empty repo reserves it on first push).
- [ ] Test in a clean chroot (`extra-x86_64-build` from `devtools`), then
      run the release job and verify both first pushes. The binary package is
      published at `ssh://aur@aur.archlinux.org/opal-media-player-bin.git`.
...
- [x] `scripts/install.sh` is the official entry point (README leads with it):
