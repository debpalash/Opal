...
| Non-blocking playback enrichment | Taste events are buffered and the per-play time preference is copied to an owned worker, so SQLite enrichment never delays `loadfile`; both sinks reject incognito activity |
