...
| Fast deterministic media switching | The outgoing URL, position, and episode binding are copied before player state changes; the new request reaches mpv before SQLite/server resume bookkeeping. Resume writes run on owned workers behind a per-identity latest-sequence gate, so they neither block frame presentation nor let scheduling reorder progress. `load-issued` is stamped at the real mpv/resolver handoff |
