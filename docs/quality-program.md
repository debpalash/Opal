...
| Portable quality inventory | Windows runs in UTF-8, source paths are normalized, POSIX-only execution probes are separated from static contracts, and an explicit missing `--database` path is populated from the shipped schema as a disposable fixture; current isolated report: 431 passed, 0 failed, 12 environment/optional-component skips, 0 warnings |
...
| Non-blocking playback enrichment | Taste events are buffered behind a race-free single-flusher latch; vector ingestion and the per-play time preference use owned workers, so SQLite enrichment never delays `loadfile`; persistence sinks reject incognito activity |
| Fast deterministic media switching | The outgoing URL, position, and episode binding are copied before player state changes; the new request reaches mpv before SQLite/server resume bookkeeping, preserving exact progress while overlapping that work with demux/decode. `load-issued` is stamped at the real mpv/resolver handoff |
