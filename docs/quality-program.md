...
| Identity-safe movie removal | Resume rows retain the verified TMDB movie ID across direct and torrent playback without title matching. Removing that native history item queues exact Trakt and SIMKL history removal, and the same per-movie state identity supersedes any older pending completion. |
