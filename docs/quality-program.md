...
| Bounded shared artwork decode | One shared cover decoder probes untrusted image headers before stb allocation, rejects invalid/overflowing dimensions and caps each decoded RGBA cover at 24 MiB (192 MiB at the eight-worker ceiling); the poster daemon, queue, anime, Jellyfin, plugin and YouTube artwork paths all use it |
