...
| Credential-safe self-hosted sign-in | Jellyfin, Audiobookshelf and OPDS login forms send server/user/password fields only in authenticated POST bodies; route handlers reject GET, prefer the body over query data, and scrub temporary password buffers after dispatch |
...
| Jellyfin user-state parity | Browse snapshots carry authoritative resume, favorite and watched state; web cards expose accessible one-click actions backed by authenticated stable-ID mutations, optimistic generation guards and failure rollback without blocking the UI or HTTP responder |
| Stable Plex web playback | Plex cards expose server-owned duration, watched and resume state; playback POSTs the rating key and resolves it against the current page at execution time, preventing a stale paginated index from opening the wrong item |
| Convergent Plex watched state | Native and web library rows toggle watched state by validated rating key through Plex's documented PUT endpoints; token headers never enter URLs, mutation workers own copied credentials, and generation-checked optimistic rollback cannot overwrite a newer click or browse page |
| Convergent Plex ratings | Server-owned 0–10 ratings render in native and web libraries and accept half-step updates through Plex's documented PUT endpoint; finite-range validation, header-only credentials, owned workers and an action-specific generation guard preserve the latest user intent |
| Unified cross-content details | Plex, Jellyfin, Audiobookshelf, OPDS, podcast, comic, novel, visual-novel, drama and RSS rows open one responsive semantic dialog with available source metadata and correct open/read/play/resume/account actions; artwork is HTTP(S)-allowlisted and native modal focus trapping, Escape, outside-click close and trigger-focus restoration align mouse, touch and keyboard behavior |
| Plex hierarchical browsing | Native and web clients drill through shows→seasons→episodes and artists→albums→tracks using bounded, generation-checked navigation; paging jobs own copied request values and Back refetches the exact parent context |
...
| AniList account and progress sync | The official desktop/PIN OAuth flow is exposed without returning stored tokens to the browser. Long JWTs are encrypted at rest on Windows, AniList IDs survive browse caching, episode playback enters the durable latest-value outbox, and queued progress has visible retry/revoke controls. Progress continues coalescing after revocation and resumes after reconnection. |
| Actionable AniList revocation | AniList delivery captures the real HTTP status separately from response content. A 401 disables futile automatic retries, preserves queued progress, publishes reconnect-required state, and resumes the same durable queue after a replacement token is saved. |
...
| Identity-bound movie completion | A verified TMDB movie identity survives the metadata-to-resolver-to-player handoff and is cleared on unrelated loads. Trakt and SIMKL receive one durable completion only after 90% real playback advancement; seeking to credits cannot mark a movie watched. |
| Convergent episode history | Native and web watched toggles plus automatic playback completion share one local mutation seam. Trakt and SIMKL receive both add and remove history operations, while an atomic per-episode outbox identity replaces stale offline toggles so reconnect always delivers the user's newest state. |
| Identity-safe movie removal | Resume rows retain the verified TMDB movie ID across direct and torrent playback without title matching. Native and web removal cross a bounded acknowledged UI-thread command seam, queue exact Trakt and SIMKL history removal, and use the same per-movie state identity to supersede any older pending completion. |
| Durable library view state | Native Watching type/status filters persist through the shared configuration store. The web library validates and restores type/status filters, smart/recent/title/progress sorting, and comfortable/compact density locally; malformed stored values fall back safely and real update timestamps drive recent order. |
| Bounded library rendering | Web Watching requests and renders a persisted 24/48/96-item server page, clamps stale page state after filtering or removal, and provides keyboard-accessible previous/next navigation. Filtering and sorting happen before slicing; large libraries no longer create an unbounded payload or DOM. |
| Off-screen card virtualization | Web browse, recommendation, and Watching cards use `content-visibility: auto` with intrinsic-size placeholders, avoiding layout and paint work outside the viewport without scrollbar jumps. Native high-volume grids retain their row-window virtualization. |
| Enforced web performance budget | The client measures boot-to-painted-shell, Event Timing interaction p95, and bounded long-task samples in session memory. Diagnostics evaluates them against explicit 1.5 s shell, 100 ms interaction-p95 and 250 ms long-task ceilings with a visible pass/fail verdict. Measurements retain no URLs, control labels, or user content. Polls, DOM lists, payloads, and retained samples remain bounded. |
| Explicit web download-file actions | Activity exposes Reveal on the desktop host and separately confirmed permanent deletion. The server accepts only direct children of the configured download root, rejects traversal, absolute paths, links and special entries, verifies existence, and requires the literal `confirm=DELETE` capability for disk removal. |
| Web transfer-history ownership | Activity lists stable SQLite row identities and supports confirmed per-entry or complete history cleanup without touching downloaded files. Desktop mutations are acknowledged after the UI thread updates SQLite and its native cache; headless applies under the same bounded producer lock. |
